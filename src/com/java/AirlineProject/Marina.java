...
public class Marina {
    ...
    public void Welcome() {
        System.out.println("\n\t\t\tWelcome to Javengers Airline");
        System.out.println("\nPlease choose the type of trip: 1) one-way 2) round 3) multi-city");
...
//If it's a multi-city trip then I should not be able to choose the same number twice in a row.
    ...
    public void sub(int i){
        if(i<10){
            totalCities.add(db.getCities()[i-1].substring(2));
        }
        else{
            totalCities.add(db.getCities()[i-1].substring(3));
        }
    }
